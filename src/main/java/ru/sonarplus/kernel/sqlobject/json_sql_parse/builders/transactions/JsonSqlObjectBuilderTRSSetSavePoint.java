...
import static ru.sonarplus.kernel.sqlobject.json_sql_parse.builders.JsonSqlObjectBuilderConsts.KIND_TRS_SET_SAVEPOINT;

public class JsonSqlObjectBuilderTRSSetSavePoint extends JsonSqlObjectBuilder {
    @Override
    public SqlObject parseJsonNode(JSONObject node) {
        SqlTransactionCommand trs = new SqlTransactionCommand();
        trs.command = SqlTransactionCommand.TransactionCommand.SET_SAVEPOINT;
        trs.statement = (String) node.get(KEY_TRS_SAVEPOINT_NAME);
        return trs;
    }

    @Override
    public String getKind() { return KIND_TRS_SET_SAVEPOINT; }
